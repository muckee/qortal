# Qortal-v6.0 - MAJOR UPDATE (Devnet)

**Upcoming Qortal Changes | Network Upgrades** **MAJOR Qortal Networking and QDN Update** **Primary Developers:** Phil, Ice

This is the next MAJOR version update for Qortal that is being pushed to the repo now. It is currently in the testing phase on the Qortal Devnet.
...
* **Peer Context:** Peers carry context (NETWORK vs NETWORKDATA). A single Peer object can exist in both networks with separate message queues and behaviors.
...
* **Capability Discovery:** Only connects to peers advertising QDN support. It parses the port from the capability map (supporting both Integer and Long types).
...
**New Settings Added:**
* `qdnListenPort`: **12394 (Default)**.
* `maxDataPeerConnectionTime`: Defaults to 1800s (30 minutes). Limits the lifetime of temporary data connections to prevent resource exhaustion.
...
* **Relay Metadata:** Fixed message parsing for `PeerDataMessage` to correctly extract relay metadata.
...
* **Batch Block Syncing:** Peers more than 200 blocks behind use the new `GetBlocks` message to request 200 blocks at once, increasing sync performance by up to **300%**.
