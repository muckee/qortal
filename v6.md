...
* **`GET /arbitrary/resource/request/peers/service/name`**: Retrieve information about a QDN asset that is currently being transferred.
* **`POST /data/cache/erase`**: Removes the QDN temporary cache
* **`GET /data/cache/size`**: Returns the size of the current QDN temporary cache.
* **`GET /add/status`**: Now includes number of QDN connects as a separate entry.
---
...
* **QDN QApp Build Process:** QApps build time was reduced by decrypting on the fly and then assembly.
---
